...
function App() {
  const [activePage, setActivePage] = useState<'landing' | 'pricing' | 'my-report' | 'dashboard' | 'settings' | 'admin'>('landing');
  ...
  // If on Landing Page, render separate layout
  if (activePage === 'landing') {
    return (
      <LandingPage 
        onLogin={() => setActivePage('my-report')} 
        onNavigate={(page) => setActivePage(page as any)}
      />
    );
  }

  // If on Pricing Page, render separate layout
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-200 flex font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0">
         ...
         <div className="lg:hidden bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 p-4 flex items-center justify-between sticky top-0 z-30">
